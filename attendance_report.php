...
<div id="table_clone" style="display: none">
	...
	<table class='table table-bordered table-hover att-list'>
        ...
        <!-- <th>Actions</th> Button for Attendance Percentage -->
			</tr>
		</thead>
		<tbody>
		</tbody>
...
<style>
	.present-inp,.absent-inp,.present-lbl,.absent-lbl{
		cursor: pointer;
	}
</style>
<noscript>
...
<script>
					...
					if(Object.keys(data).length > 0){
						...
						Object.keys(data).map(function(k){
							...
							var td = '<td class="text-center">';
								...
								td += absent;
								td += '</td>';
								 // Calculate attendance percentage and add it to the row
                        var attendancePercentage = ((present / (present + absent)) * 100).toFixed(2);
                        td += '<td class="text-center">';
                        td += attendancePercentage + '%';
                        td += '</td>';
						// Add button to show the attendance percentage dynamically
                        // td += '<td class="text-center">';
                        // td += '<button type="button" class="btn btn-info btn-sm show-attendance" data-student-id="' + id + '">Show Attendance</button>';
                        // td += '</td>';
	...
	$('.calculate-attendance').click(function() {
  var studentId = $(this).data('student-id');
  var classSubjectId = $('#class_subject_id').val();

  $.ajax({
    url: 'ajax.php?action=get_attendance_percentage',
    method: 'POST',
    data: {
    ...
    success: function(response) {
      var attendancePercentage = parseFloat(response).toFixed(2);
      $(this).closest('tr').find('.attendance-percentage').text(attendancePercentage + '%');
    }
  });
});
</script>