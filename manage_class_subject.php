    ...
    <form action="" id="manage-class_subject">
        ...
        <div class="form-group">
            ...
            <select name="class_id" id="" class="custom-select select2">
                <option value=""></option>
                <?php
                $class = $conn->query("SELECT c.*,concat(co.course,' ',c.level,'-',c.section) as `class` FROM `class` c inner join courses co on co.id = c.course_id order by id asc");
        ...
        <div class="form-group">
            ...
            <select name="faculty_id" id="" class="custom-select select2">
                <option value=""></option>
                <?php
                $class = $conn->query("SELECT * FROM faculty order by id asc");
                while($row=$class->fetch_assoc()):
                ?>
        ...
        <div class="form-group">
            ...
            <select name="subject_id" id="" class="custom-select select2">
                <option value=""></option>
                <?php
                $class = $conn->query("SELECT * FROM subjects order by id asc");
                while($row=$class->fetch_assoc()):
                ?>
